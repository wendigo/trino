...
public final class MapVectorWriter
        implements ArrowVectorWriter
{
    ...
    public MapVectorWriter(MapVector vector, MapType type)
    {
        this.vector = requireNonNull(vector, "vector is null");

        if (vector.getDataVector() instanceof StructVector structVector) {
            this.keyWriter = createArrowWriter(structVector.getChildByOrdinal(0), type.getKeyType());
            this.valueWriter = createArrowWriter(structVector.getChildByOrdinal(1), type.getValueType());
        }
        else {
